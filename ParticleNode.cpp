...
void ParticleNode::computeVertices() const
{
	...
	for (const auto& particle : particles)
	{
		...
		float ratio = particle.lifetime.asSeconds() / TABLE.at(type).lifetime.asSeconds();	// the older the particle => more transparent it is
		color.a = static_cast<sf::Uint8>(255 * std::max(ratio, 0.f));						//alpha number = how transparent it is	
		
		//four vertices for each particle
		//first two args = target coordinates
		//second two = texture coordinates
		//fifth = vertex colour
